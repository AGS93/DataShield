...
namespace BlacklistUpdater
{
    static class Updater
    {
         ...
         * 
         * This application allows an administrator to update the process blacklist table on the database. 
         * This is done by reading a text file containing all the blacklisted processes and updating the 
         * 'blacklist' table.
         * 
         * Param: Nome.
         * Return: None.
                ...
                if (!CheckBanned())
                {
                    if (PasswordProtection())
                    {
                        ...
                        if (ofd.ShowDialog() == DialogResult.OK)
                        {
                            try
                            {
                                if ((blacklist = File.ReadAllLines(ofd.FileName)) != null)
                                {
                                    String wl = String.Join(", ", blacklist);
                                    using (WebClient client = new WebClient())
                                    {
                                        ServicePointManager.ServerCertificateValidationCallback = (senderX, certificate, chain, sslPolicyErrors) => { return true; };
                                        String message = client.UploadString(ConfigurationManager.AppSettings["updateblacklist"], wl);
         ...
         * looks up the product name, being 'blacklistupdater' and checks that the provided HWID
        ...
        public static Boolean CheckHWID()
        {
            ...
            {
                ServicePointManager.ServerCertificateValidationCallback = (senderX, certificate, chain, sslPolicyErrors) => { return true; };
                response = client.UploadString(ConfigurationManager.AppSettings["checkhwid"], "blacklistupdater" + "|" + hwid);
            }
        ...
        public static Boolean PasswordProtection()
        {
            ...
            while (granted == false)
            {
                ...
                {
                    ServicePointManager.ServerCertificateValidationCallback = (senderX, certificate, chain, sslPolicyErrors) => { return true; };
                    response = client.UploadString(ConfigurationManager.AppSettings["checkupdateblacklist"], attempt);
                }
                if (attempts < 2)
                {
